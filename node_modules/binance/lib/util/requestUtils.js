...
//   throw new Error(msg);
// }
function getOrderIdPrefix(network, prefixVersion) {
    switch (network) {
        case 'spot':
        case 'spot1':
        case 'spot2':
...
function requiresWSAPINewClientOID(request, wsKey) {
    switch (wsKey) {
        ...
        case websocket_util_1.WS_KEY_MAP.usdmMarket:
        case websocket_util_1.WS_KEY_MAP.usdmPublic:
        case websocket_util_1.WS_KEY_MAP.usdmPrivate:
        case websocket_util_1.WS_KEY_MAP.usdmTestnet:
        case websocket_util_1.WS_KEY_MAP.usdmTestnetMarket:
        case websocket_util_1.WS_KEY_MAP.usdmTestnetPublic:
        case websocket_util_1.WS_KEY_MAP.usdmTestnetPrivate:
        ...
        case websocket_util_1.WS_KEY_MAP.marginUserData:
        case websocket_util_1.WS_KEY_MAP.alpha:
            return false;
        default: {
...
function validateWSAPINewClientOID(request, wsKey) {
    ...
    for (const orderIdProperty of newClientOIDProperties) {
        ...
        if (!request.params[orderIdProperty].startsWith(expectedOrderIdPrefix1) &&
            !request.params[orderIdProperty].startsWith(expectedOrderIdPrefix2)) {
            logInvalidOrderId(orderIdProperty, expectedOrderIdPrefix2, request.params);
        }
    }
}
function serialiseParams(params = {}, strict_validation = false, encodeValues = false, filterUndefinedParams = false, sortProperties = false) {
...
function getRESTRequestSignature(data, options, key, secret, timestamp) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (key && secret) {
            ...
            const signMethodPEM = 'base64'; // needed for RSA/Ed25519
            ...
            if (typeof options.customSignMessageFn === 'function') {
                ...
                signature = yield (0, webCryptoAPI_1.signMessage)(serialisedParams, secret, signMethod, signAlgorithm, signMethodPEM);
...
function getRestBaseUrl(clientType, restClientOptions) {
    ...
    if (restClientOptions.demoTrading) {
        ...
        throw new Error(`Demo trading is currently not supported for the product group "${urlKey}". If demo trading should be available here, please open an issue on GitHub.`);
    }
 ...
 */
function appendEventIfMissing(
// eslint-disable-next-line @typescript-eslint/no-explicit-any
wsMsg, wsKey, eventType) {
    if (wsMsg.e) {
        return;
    }
    if (eventType) {
