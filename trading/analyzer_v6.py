...
def check_risk_controls(state, trade_log, epic, direction, open_positions):
    ...
    # 3. Consecutive loss controls (DISABLED during testing phase — keep trading to gather data)
    consec_losses = state.get('consecutive_losses', 0)
    size_mult = 1.0
    # Testing phase: no pause, no stop, no size reduction
    # TODO: Re-enable when switching to live trading

    # 4. Correlated instrument limit (US indices same direction max 2)
    ...
    # 5. Max 1 position per instrument
    for p in open_positions:
        pos_epic = p['position'].get('epic', p['market'].get('epic', ''))
        if pos_epic == epic:
...
def _save(report, trade_log, state, positions, args, all_indicators, account_data):
    ...
    # Also write to worker dashboard.json (single source of truth)
    wp = os.path.expanduser('~/.openclaw/workspace/workers/forex-trader/dashboard.json')
    try:
        with open(wp, 'w') as f:
            json.dump(dashboard, f, indent=2)
    except:
